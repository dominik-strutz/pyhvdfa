! ==============================================================================
! modules_patched.f90 — thread-safe copy of HV-DFA/modules.f90
!
! Source: HV-DFA/modules.f90 by Antonio García-Jerez & José Piña-Flores
!   https://github.com/agarcia-jerez/HV-DFA
!   García-Jerez et al. (2016), Computers & Geosciences, 97, 67–78
!   https://doi.org/10.1016/j.cageo.2016.06.016
!
! This file is a local copy of the upstream modules.f90 (which lives in
! the read-only git submodule fortran_src/HV-DFA/) with one addition:
! !$OMP THREADPRIVATE directives on all mutable module variables, giving
! each OS thread its own private copy of the module state and enabling
! safe concurrent calls to COMPUTE_HV from multiple Python threads.
!
! If the submodule is ever updated, verify this file is still in sync:
!   diff <(grep -v 'OMP THREADPRIVATE' fortran_src/modules_patched.f90 | \
!          grep -v '^! ===' | grep -v '^! Source' | grep -v '^!   https' | \
!          grep -v '^!   Garcia' | grep -v '^! This file' | \
!          grep -v '^! the read' | grep -v '^! the git' | \
!          grep -v '^! each OS' | grep -v '^! safe con' | \
!          grep -v '^! If the' | grep -v '^!   diff') \
...
! ==============================================================================
      ...
      REAL SHDAMP,PSVDAMP
      !$OMP THREADPRIVATE(UNIT_RDPGS,SHDAMP,PSVDAMP)
      !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ! I N T E R F A C E S
      !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      interface matmul1x1
      ...
      !$OMP THREADPRIVATE(G_x1,G_x2,G_y1,G_y2,G_POLARITY,G_OMEGA,   &
      !$OMP&  G_PRECISION,X,VALUES_L,VALUES_R,VALID_L,VALID_R,        &
      !$OMP&  G_NX,G_NMODES,G_DX,G_DXTYPE,GO_DOWN,                   &
      !$OMP&  forbitVelocityInversion,G_SLOWS,G_SLOWP,                &
      !$OMP&  ALFA,BTA,H,RHO,MU,NCAPAS,G_VELOCITYINVERSION,          &
      !$OMP&  SOLVER_RELATIVESTEP,SOLVER_SEARCHSTEP,                  &
      !$OMP&  G_MAXRAYLEIGHSLOWNESS,G_SLOWSMIN,G_SLOWSMAX,           &
      !$OMP&  ILAYER,ISRAYLEIGH)
