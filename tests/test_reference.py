...
def _load_reference(tag: str) -> tuple[np.ndarray, np.ndarray]:
    ...
    if not ref_file.exists():
        pytest.skip(f"Reference file missing: {ref_file}. Run generate_reference.sh first.")
...
def test_hv_matches_fortran(tag: str):
    ...
    np.testing.assert_allclose(
        py_hv[mask], ref_hv[mask],
        rtol=RTOL, err_msg=f"H/V mismatch for model {tag!r}",
    )
...
def test_hv_result_shape_and_finitude(tag: str):
    ...
    # Check shape
    assert py_hv.shape == ref_hv.shape, (
    ...
    assert finite_count > 0.9 * len(py_hv), (
...
def test_correlation_with_fortran(tag: str):
    ...
    assert correlation > 0.95, (
        f"Low correlation for {tag}: r={correlation:.4f}"
    )
